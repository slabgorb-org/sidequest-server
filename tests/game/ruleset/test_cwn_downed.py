...
def test_non_traumatic_downed_is_mortal_only():
    core = _core()
    r = _MOD.resolve_downed(
        core=core, save_target=10, scene_traumatic=False, cfg=_CFG, rng=random.Random(1)
    )
...
def test_traumatic_downed_save_made_no_major():
    ...
    r = _MOD.resolve_downed(core=core, save_target=10, scene_traumatic=True, cfg=_CFG, rng=rng)
    assert r.mortal is True
    assert r.major is False
...
def test_downed_emits_spans():
    ...
    _MOD.resolve_downed(
        core=core, save_target=10, scene_traumatic=True, cfg=_CFG, rng=rng, _tracer=tracer
    )
...
def test_non_cwn_cfg_fails_loud():
    ...
    with pytest.raises(ValueError):
        _MOD.resolve_downed(
            core=core, save_target=10, scene_traumatic=False, cfg=None, rng=random.Random(1)
        )
