...
Pins the absolute behavior of ``NativeRulesetModule`` for stat_modifier and
...
def test_native_stat_modifier_dnd_formula(score: int, expected: int) -> None:
    assert _NATIVE.stat_modifier({"STR": score}, "STR") == expected


def test_native_stat_modifier_missing_stat_defaults_to_zero() -> None:
    assert _NATIVE.stat_modifier({}, "STR") == 0


def test_native_stat_modifier_case_insensitive() -> None:
...
def test_native_compute_dc_clamped(base: int, expected_dc: int) -> None:
