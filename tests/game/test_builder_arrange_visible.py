...
def test_arrange_visible_render_carries_ability_names():
    """_render_arrangement_message sets ability_names from the ruleset."""
    from sidequest.genre.models.character import (
        CharCreationChoice,
        MechanicalEffects,
    )

    flavor_names = ["Strength", "Agility", "Endurance", "Insight", "Spirit", "Harmony"]
    rules = RulesConfig(
        ability_score_names=flavor_names,
        stat_generation="roll_3d6_arrange_visible",
    )
    scenes = [
        CharCreationScene(
        ...
        CharCreationScene(
            ...
            choices=[
                CharCreationChoice(
                    label="Channeler",
                    description="Harmony is your prime.",
                    mechanical_effects=MechanicalEffects(class_hint="Channeler"),
                ),
            ],
            allows_freeform=False,
        ),
        ...
        CharCreationScene(
            id="the_kit", title="Kit", narration="...", choices=[], allows_freeform=False
        ),
        CharCreationScene(
            id="the_mouth", title="Mouth", narration="...", choices=[], allows_freeform=False
        ),
    ]
    ...
    builder.with_classes(
        [
            ClassDef(
                ...
                prime_requisite="Harmony",
                minimum_score=9,
                kit_table="channeler_kit",
            )
        ]
    )
    ...
    msg = builder.to_scene_message("player-1")
    assert msg.payload.ability_names == flavor_names
