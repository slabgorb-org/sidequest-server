...
def store(monkeypatch, migrated_db: str):
    ...
    slug = f"locres_{uuid.uuid4().hex[:8]}"
...
def test_proactive_match_real_object_returns_resolved(store) -> None:
    res = resolve(
        store=store,
    ...
    assert store.list_location_promotions(region_id="the_glenross_arms") == []
...
def test_player_initiated_miss_mints_yes_and_entity(store) -> None:
...
def test_player_initiated_match_does_not_mint(store) -> None:
...
def test_flavor_only_engaged_mechanically_promotes(store) -> None:
...
def test_flavor_only_mention_does_not_promote(store) -> None:
...
def test_definite_article_stripped_when_matching(store) -> None:
...
def test_indefinite_articles_stripped(store) -> None:
...
def test_match_is_case_insensitive(store) -> None:
    res = resolve(
        store=store,
        region_id="the_glenross_arms",
        authored_entities=_entities(),
...
def test_authored_yaml_never_mutates_on_promotion(store) -> None:
...
def test_authored_yaml_never_mutates_on_mint(store) -> None:
...
def test_existing_promotion_layers_on_top_of_authored(store) -> None:
...
def test_minted_entity_visible_on_subsequent_resolve(store) -> None:
...
def test_promotions_in_other_region_do_not_match(store) -> None:
