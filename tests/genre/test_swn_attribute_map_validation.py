...
def test_swn_pack_with_complete_map_validates():
    rc = RulesConfig(ruleset="swn", ability_score_names=SIX, swn=SwnConfig(attribute_map=GOOD_MAP))
...
def test_swn_pack_missing_one_key_fails_loud():
    ...
    with pytest.raises(ValidationError, match="WISDOM"):
        RulesConfig(ruleset="swn", ability_score_names=SIX, swn=SwnConfig(attribute_map=partial))
