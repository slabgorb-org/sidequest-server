"""caverns_and_claudes → WWN — real-chargen caster casts through the PRODUCTION seam.

Originally Plan-3 Task 8's "cast a damage spell into combat through the narrator
...
This test keeps what it uniquely proves — that a ``Mage`` built through the REAL
``CharacterBuilder.build()`` walk on the real caverns_and_claudes pack is seeded
with a usable ``SpellcastingState`` from the class's ``starting_prepared`` — and
proves that seeded caster's spell resolves end-to-end through the production dice
seam:

  1. real chargen seeds ``spellcasting`` (prepared == wracking_bolt, sleepless_tithe;
     2 casts);
...
def test_wwn_cast_spell_routes_through_wwn_module_on_real_caverns_and_claudes(
    ...
    # ablated, below the 10-HP kill; the server reprisal rolls min and misses.
    monkeypatch.setattr("random.randint", lambda a, b: a)

    dispatch_dice_throw(
        payload=DiceThrowPayload(
            request_id="req-158-47-cc",
    ...
    assert sc_after.casts_remaining == casts_before - 1, (
        f"casting wracking_bolt must spend exactly one cast through the production "
