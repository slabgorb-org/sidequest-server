...
# ---------------------------------------------------------------------------
# ensure_loaded — cross-world bestiary bleed purge (story 158-33)
# ---------------------------------------------------------------------------
#
# barsoom's persisted Manual carried encounter enemies authored ONLY in the
# SIBLING world long_foundry's bestiary (a stale seed from the genre-tier-bestiary
# era, pre-ADR-120). They are class="creature", so the existing native-class purge
# (purge_ruleset_incoherent_encounters) does NOT catch them. ensure_loaded must
...
# path — we need an explicit class="creature" enemy here.
def _world_creature_encounter(enemy_name: str, *, tier: int = 2, label: str | None = None):
    return ManualEncounter(
        data={"enemies": [{"name": enemy_name, "class": "creature", "tier": tier, "hp": 9}]},
        label=label or f"1x {enemy_name} (tier {tier})",
        tier=tier,
        state=EntryState.AVAILABLE,
    )


def _pack_with_world_bestiary(world_slug: str, *creature_names: str, ruleset: str = "wwn"):
    """A pack stand-in whose ``effective_bestiary(world)`` returns ONLY
    ``creature_names`` — i.e. the world's own scoped roster. No ``source_dir`` so
    ``ensure_loaded`` skips re-seeding and the purge is tested in isolation.
    """
    from types import SimpleNamespace
    ...
    entries = [
        ...
        for n in creature_names
    ]
    bestiary = Bestiary(entries=entries) if entries else None
    pack = SimpleNamespace(
        worlds={world_slug: SimpleNamespace(authored_npcs=[])},
        rules=SimpleNamespace(ruleset=ruleset, combat_encounters=True),
    )
    pack.effective_bestiary = lambda _world: (bestiary, "world")
    return pack


def _barsoom_manual_with_foreign_encounter(tmp_path: Path) -> None:
    """Persist a barsoom Manual: 4 walk-ons + an in-world Banth encounter + a
    foreign long_foundry 'Knight of the Ashen Banner' encounter. After the foreign
    purge the Banth encounter remains, so ``needs_seeding()`` stays False and no
    re-seed masks the assertion.
    """
    m = MonsterManual(
        genre="heavy_metal",
        world="barsoom",
        npcs=[_human(f"Red Martian {i}") for i in range(4)],
        encounters=[
            _world_creature_encounter("Banth", label="canon"),
            _world_creature_encounter("Knight of the Ashen Banner", label="foreign"),
        ],
    )
    assert not m.needs_seeding()
    m.save()


def test_ensure_loaded_purges_foreign_bestiary_encounter(tmp_path: Path) -> None:
    sd = _FakeSessionData(
        genre_slug="heavy_metal",
        world_slug="barsoom",
        genre_pack=_pack_with_world_bestiary("barsoom", "Banth"),
    )
    with mock.patch(
        "sidequest.game.monster_manual.MonsterManual._manuals_dir", return_value=tmp_path
    ):
        _barsoom_manual_with_foreign_encounter(tmp_path)
        loaded = monster_manual_inject.ensure_loaded(sd)

        assert loaded is not None
        labels = {e.label for e in loaded.encounters}
        assert "foreign" not in labels, "sibling-world creature must be purged on load"
        assert "canon" in labels, "the world's own creature must survive (don't empty the pool)"

        # Persisted — the next process must not re-inherit the foreign creature.
        reloaded = MonsterManual.load("heavy_metal", "barsoom")
    assert "Knight of the Ashen Banner" not in str(reloaded.model_dump())
    assert any(e.label == "canon" for e in reloaded.encounters)


def test_ensure_loaded_keeps_pure_in_world_manual_untouched(tmp_path: Path) -> None:
    # AC-2 at the wiring seam: a Manual whose encounters are all in-world must not
    # lose any (and must not fire the purge span — see span test below).
    sd = _FakeSessionData(
        genre_slug="heavy_metal",
        world_slug="barsoom",
        genre_pack=_pack_with_world_bestiary("barsoom", "Banth", "White Ape"),
    )
    with mock.patch(
        "sidequest.game.monster_manual.MonsterManual._manuals_dir", return_value=tmp_path
    ):
        MonsterManual(
            ...
            npcs=[_human(f"Red Martian {i}") for i in range(4)],
            encounters=[
                _world_creature_encounter("Banth", label="a"),
                _world_creature_encounter("White Ape", label="b"),
            ],
        ).save()
        loaded = monster_manual_inject.ensure_loaded(sd)
    assert loaded is not None
    assert {e.label for e in loaded.encounters} == {"a", "b"}


def test_ensure_loaded_foreign_purge_emits_span(tmp_path: Path, otel_capture) -> None:
    sd = _FakeSessionData(
        genre_slug="heavy_metal",
        world_slug="barsoom",
        genre_pack=_pack_with_world_bestiary("barsoom", "Banth"),
    ...
    assert attrs["purged"] == 1


def test_ensure_loaded_no_foreign_purge_span_when_clean(tmp_path: Path, otel_capture) -> None:
    sd = _FakeSessionData(
        genre_slug="heavy_metal",
        world_slug="barsoom",
        genre_pack=_pack_with_world_bestiary("barsoom", "Banth"),
    ...
    with mock.patch(
        ...
        MonsterManual(
            ...
            world="barsoom",
            npcs=[_human(f"Red Martian {i}") for i in range(4)],
            encounters=[_world_creature_encounter("Banth", label="canon")],
        ).save()
        monster_manual_inject.ensure_loaded(sd)
    ...
    assert fired == [], "no span should fire when nothing is foreign (no empty spans)"
