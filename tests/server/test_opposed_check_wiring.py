...
# ---------------------------------------------------------------------------
# RW-2 telemetry honesty (playtest 2026-06-05, the_circuit chase)
# ---------------------------------------------------------------------------
#
# (a) The dice span/log stamped a flat-DC outcome (e.g. CritSuccess for a
#     total of 16) on rolls whose beat application was DEFERRED to the
#     opposed resolver — the span lied to the GM panel: the real tier comes
#     from ``opposed_roll_resolved`` once the opponent's roll lands.
# (b) When the deferred player roll is never consumed (zero opponent beat
#     selections survive the narration turn), the stash was cleared SILENTLY
#     by the session handler — the player's dice vanished with no GM-panel
#     trace. That silence is what let the entire engine die unnoticed.


def _capture_emitter_events(monkeypatch) -> list[tuple[str, dict]]:
    from sidequest.telemetry.spans.emitter import Emitter

    captured: list[tuple[str, dict]] = []
    monkeypatch.setattr(
        Emitter,
        "fire",
        staticmethod(lambda name, attrs: captured.append((name, dict(attrs)))),
    )
    return captured


def test_dice_result_broadcast_marks_deferred_on_opposed_path(monkeypatch, caplog):
    """The opposed-pending dispatch must stamp ``deferred_opposed=True`` on
    the ``dice.result_broadcast`` event and the ``dice.throw_resolved`` log —
    the flat-DC outcome on that path is provisional, not final."""
    import logging

    captured = _capture_emitter_events(monkeypatch)
    caplog.set_level(logging.INFO, logger="sidequest.server.dispatch.dice")
    ...
    events = [attrs for (name, attrs) in captured if name == "dice.result_broadcast"]
    assert len(events) == 1
    assert events[0]["deferred_opposed"] is True, (
        "the dice.result_broadcast event must mark the outcome as deferred on "
        "the opposed path — the stamped tier is provisional until "
        "opposed_roll_resolved fires"
    )
    assert "deferred_opposed=True" in caplog.text


def test_dice_result_broadcast_not_deferred_on_legacy_path(monkeypatch, caplog):
    """Legacy beat_selection rolls resolve immediately — the event must say so."""
    import logging
    ...
    events = [attrs for (name, attrs) in captured if name == "dice.result_broadcast"]
    assert len(events) == 1
    assert events[0]["deferred_opposed"] is False
    assert "deferred_opposed=False" in caplog.text


def _capture_watcher_events(monkeypatch) -> list[tuple[str, dict, dict]]:
    from sidequest.server import narration_apply
    ...
    monkeypatch.setattr(
        narration_apply,
        "_watcher_publish",
        lambda event_type, fields, **kw: published.append((event_type, dict(fields), kw)),
    )
    return published


def test_unconsumed_pending_roll_emits_loud_watcher_warning(monkeypatch):
    """opposed_check encounter live + a stashed player d20 + ZERO surviving
    beat selections → the roll is about to be silently discarded (the
    session handler clears the stash after apply). narration_apply must emit
    a loud ``opposed_check_pending_roll_unconsumed`` warning first."""
    published = _capture_watcher_events(monkeypatch)
    ...
    # Narrator emitted NO beat selections at all (the SDK-zeroing failure
    # mode, or a narrator that simply didn't pick an opponent beat).
    result = NarrationTurnResult(narration="The patrol surges.", beat_selections=[])
    ...
    unconsumed = [
        (fields, kw)
        for (event_type, fields, kw) in published
        if event_type == "state_transition"
        and fields.get("op") == "opposed_check_pending_roll_unconsumed"
    ]
    assert len(unconsumed) == 1, (
        "the about-to-be-discarded player roll must surface on the GM panel — "
        f"got watcher events: {[(f.get('op'), f.get('kind')) for (_, f, _) in published]}"
    )
    fields, kw = unconsumed[0]
    assert fields["encounter_type"] == "combat"
    assert fields["player_d20"] == 16
    assert fields["player_beat_id"] == "attack"
    assert kw.get("severity") == "warning"
    assert kw.get("component") == "encounter"
    # No dial moved — the resolver did not run.
    assert enc.player_metric.current == 0
    assert enc.opponent_metric.current == 0


def test_no_unconsumed_warning_when_resolver_consumes_the_roll(monkeypatch):
    """Negative: when an opponent beat selection survives and the resolver
    runs, the roll WAS consumed — no unconsumed warning may fire."""
    published = _capture_watcher_events(monkeypatch)
    _patched_d20(monkeypatch, value=5)
    ...
    assert not any(
        fields.get("op") == "opposed_check_pending_roll_unconsumed" for (_, fields, _) in published
    )


def test_no_unconsumed_warning_without_pending_roll(monkeypatch):
    """Negative: no stashed d20 → nothing to lose → no warning (the
    awaiting-dice path has its own info-level event)."""
    published = _capture_watcher_events(monkeypatch)
    ...
    result = NarrationTurnResult(narration="Circling.", beat_selections=[])
